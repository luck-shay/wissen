...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === "release") {
      ...
      // Check the 10 AM rule for non-designated bookings.
      const canBook = isNonDesignatedBookingAllowed(date);
      if (!canBook) {
        return NextResponse.json({ error: "Booking for future non-designated days is only allowed after 10 AM of the prior work day." }, { status: 403 });
      }
